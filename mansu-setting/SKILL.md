...
- Check source skill freshness for Ouroboros, gstack, Oh My / OMO / OMC, and addyosmani/agent-skills style skills.
...
- `source-check`: report whether Ouroboros, gstack, Oh My / OMO / OMC, and addyosmani/agent-skills source skills look stale against installed repos/adapters; make no changes unless the user also asked for update.
...
- whether Ouroboros, gstack, Oh My / OMO / OMC, and addyosmani/agent-skills source skill names still match the Mansu source catalog
...
7. Confirm `docs/mansu-manual.html` exists and includes the agent guide, family-first catalog, and selected runtime/adapter expectation.
8. Run `scripts/validate_mansu_manual.sh` and `scripts/validate_mansu_skills.sh` in the repo before syncing runtime copies.
9. Create the selected runtime skill directory if missing and safe.
10. Sync every top-level `mansu-*` directory into `<runtime-skill-dir>/<skill-name>`.
11. Copy `docs/mansu-manual.html` into `<runtime-skill-dir>/mansu-manual/docs/mansu-manual.html` so the installed manual skill has its HTML artifact.
12. Refresh gstack and Oh My source skill links/copies into the selected runtime target after their installers run.
13. Do not copy `.git`, unrelated root docs, or non-skill files into individual skill folders. The manual HTML is the only root-doc exception.
14. Confirm the installed `mansu-setting` path can be read.
15. Confirm the installed `mansu-project-start` can use Ouroboros later, but do not start a project here.
...
8. If source skills changed in a way that affects Mansu routing, use `mansu-source-curator` to update `SOURCE_SKILL_CATALOG.md`, `SOURCE_SKILL_LOCK.json`, document/code-order references, validators, and worklog before relying on the new assumptions.
9. Confirm `docs/mansu-manual.html` still matches the updated routing assumptions; if runtime/source routing changed, update it before syncing.
10. Re-run `scripts/validate_mansu_manual.sh` and `scripts/validate_mansu_skills.sh`.
11. Sync all top-level `mansu-*` directories into the selected runtime skill directory only after source curation and validation pass.
12. Copy `docs/mansu-manual.html` into `<runtime-skill-dir>/mansu-manual/docs/mansu-manual.html` whenever the manual changed or runtime copies are refreshed.
13. Refresh gstack skill links from the installed gstack repo into the selected runtime skill directory.
14. Refresh Oh My / OMO / OMC source skill links or copies after the matching source is installed and the selected runtime target expects it.
15. Do not automatically install or copy addyosmani/agent-skills into runtime skill directories unless the user asks; use it as a source-reference family by default.
16. Report version changes, installs performed, source skill freshness, manual readiness, reference curation status, and compatibility notes.
