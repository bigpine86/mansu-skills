...
require_literal "$MODEL" '`mansu-project-start/SKILL.md` | executable Zero-to-PLAN workflow' 'operating model lost project-start ownership boundary'
...
require_contains "$AGENTS_TEMPLATE" 'current phase implementation or refactor with slices -> `mansu-tdd-total`' 'AGENTS template missing tdd-total route'
require_contains "$AGENTS_TEMPLATE" 'unclear failure, regression, stack trace, or broken behavior -> `mansu-debug-rootcause`' 'AGENTS template missing debug route'
require_contains "$AGENTS_TEMPLATE" 'real-user web flow, buttons, pages, forms' 'AGENTS template missing web verification route'
require_contains "$AGENTS_TEMPLATE" 'release readiness, commit/PR/release notes' 'AGENTS template missing ship-release route'
...
require_contains "$SOURCE_CATALOG" 'Mansu-setup prepares the host before project-start uses Ouroboros' 'source catalog lost Ouroboros setup route'
...
require_contains "$SOURCE_CATALOG" 'OmO / oh-my-openagent' 'source catalog lost OmO upstream source wording'
require_contains "$SOURCE_CATALOG" 'thin distribution' 'source catalog lost LazyCodex distribution boundary'
require_contains "$SOURCE_CATALOG" 'highest-priority execution transport' 'source catalog lost LazyCodex priority scope'
require_contains "$SOURCE_CATALOG" 'fallback compatibility route' 'source catalog lost omx fallback route'
require_contains "$SOURCE_CATALOG" 'drives Codex execution' 'source catalog lost LazyCodex execution role'
require_contains "$SOURCE_CATALOG" 'Mansu owns routing, evidence, checkpoints, and final' 'source catalog lost Mansu role split'
require_contains "$SOURCE_CATALOG" 'gstack provides independent' 'source catalog lost gstack validation role'
require_contains "$SOURCE_CATALOG" 'Ouroboros execution opt-in only' 'source catalog lost Ouroboros execution guardrail'
...
require_contains "$DOC_ORDER" 'Before `mansu-tdd-total` starts, `PLAN.md` must name the active phase' 'document creation router lost tdd-total phase preflight'
