...
require_contains "$DEFINE" 'If Define is complete, tell the user the next step is `mansu-plan`' 'define must end by routing to plan'
...
require_contains "$PLAN" 'If Plan is complete, tell the user the next step is `mansu-build`' 'plan must end by routing to build'
...
require_contains "$BUILD" '`mansu-define` / `mansu-plan`' 'build must route missing plan back to define/plan'
...
require_contains "$VERIFY" 'If verification passes, run `mansu-review` next by default' 'verify must hand off to review after passing verification'
require_contains "$VERIFY" 'Do not enter `mansu-review` when verification fails' 'verify must block review after failed verification'
...
require_contains "$VERIFY_OPENAI" 'continue directly into mansu-review by default' 'verify OpenAI prompt must continue to review after pass'
