...
require_ordered_terms "$DEFINE" 'define must route missing Ouroboros to setup install mode' 'route to public `mansu-9setup`' '`install` mode' 'Ouroboros is missing'
require_contains "$DEFINE" 'public `mansu-9setup`' 'define must route stale/broken Ouroboros to setup'
require_contains "$DEFINE" '`update`/`repair` route' 'define must keep update/repair route wording'
require_contains "$DEFINE" 'MCP integration' 'define must mention broken MCP integration'
require_contains "$DEFINE" 'mcp__ouroboros\.ouroboros_interview' 'define must prefer the Codex Ouroboros MCP interview tool'
require_contains "$DEFINE" 'call `tool_search` for `ouroboros interview mcp` first' 'define must use Codex tool_search to discover Ouroboros MCP'
require_contains "$DEFINE" 'invoke that tool immediately' 'define must invoke the Ouroboros MCP tool rather than narrating discovery'
require_contains "$DEFINE" 'must run the Codex' 'define must route broken Codex MCP through setup repair'
require_contains "$DEFINE" 'then re-check `tool_search`' 'define must re-check tool_search after Codex setup repair'
require_contains "$DEFINE" 'fallback is not valid until `tool_search` failed' 'define must block fallback until tool_search fails after repair'
require_contains "$DEFINE" 'do not require the `ooo` command' 'define must not require legacy ooo command in Codex'
...
require_contains "$DEFINE" '`update`/`repair` route' 'define must route broken MCP exposure to setup update or repair'
...
require_contains "$ROOT_DIR/mansu-1define/agents/openai.yaml" 'tool_search' 'public define OpenAI prompt must call tool_search'
require_contains "$ROOT_DIR/mansu-1define/agents/openai.yaml" 'immediately invoke' 'public define OpenAI prompt must invoke Ouroboros MCP immediately when exposed'
