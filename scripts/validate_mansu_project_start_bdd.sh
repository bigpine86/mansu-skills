...
require_contains "$PROJECT" 'mcp__ouroboros\.ouroboros_interview' 'project-start must check Codex Ouroboros MCP'
require_contains "$PROJECT" 'current `ouroboros` route' 'project-start must check current Ouroboros route'
require_contains "$PROJECT" 'call `tool_search` for `ouroboros interview mcp`' 'project-start must use tool_search for Codex MCP'
require_contains "$PROJECT" 'legacy[[:space:]]*$' 'project-start must mark old Ouroboros commands as legacy compatibility'
require_contains "$PROJECT" 'compatibility' 'project-start must mark old Ouroboros commands as legacy compatibility'
