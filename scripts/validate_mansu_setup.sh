...
FILE="$ROOT_DIR/mansu-setup/SKILL.md"
OPENAI_YAML="$ROOT_DIR/mansu-setup/agents/openai.yaml"
SHIM_FILE="$ROOT_DIR/mansu-setting/SKILL.md"
SHIM_YAML="$ROOT_DIR/mansu-setting/agents/openai.yaml"
...
line_equals 1 '---' "$FILE"
line_equals 4 '---' "$FILE"
assert_contains '^name: mansu-setup$' "$FILE"
assert_contains '^description: Runtime-target-aware bootstrap/update workflow for the Mansu skill suite' "$FILE"
assert_contains 'Use for install, update, source-check, repair, runtime sync, and adapter readiness' "$FILE"
assert_contains 'Use it when installing Mansu for the first time' "$FILE"
assert_contains 'Hermes, OpenCode, Codex, or Claude Code' "$FILE"
assert_contains 'Install or update required source tools' "$FILE"
assert_contains 'Check and, when explicitly approved, prepare Open Design' "$FILE"
assert_contains 'Validate the Mansu source suite before sync and installed runtime copies after sync' "$FILE"
assert_contains 'Check source skill freshness for Ouroboros, gstack, Oh My / OMO / OMC' "$FILE"
assert_contains 'addyosmani/agent-skills style skills' "$FILE"
assert_contains 'VoltAgent/awesome-design-md design references' "$FILE"
assert_contains 'Open Design freshness' "$FILE"
assert_contains 'optional adapter compatibility' "$FILE"
assert_contains 'mansu-source-curator' "$FILE"
assert_contains '^name: mansu-setting$' "$SHIM_FILE"
assert_contains 'Compatibility alias for `mansu-setup`' "$SHIM_FILE"
assert_contains 'Use `$mansu-setup` for the actual setup' "$SHIM_FILE"
assert_contains '^# Mansu Setup Compatibility Alias$' "$SHIM_FILE"
assert_contains 'display_name: "Mansu Setup Compatibility Alias"' "$SHIM_YAML"
assert_contains 'compatibility alias only' "$SHIM_YAML"
assert_contains 'https://github.com/bigpine86/mansu-skills.git' "$ROOT_DIR/mansu-9setup/SKILL.md"
assert_contains '\$HOME/.mansu/repos/mansu-skills' "$ROOT_DIR/mansu-9setup/SKILL.md"
assert_contains '\$HOME/.codex/skills/<skill-name>' "$ROOT_DIR/mansu-9setup/SKILL.md"
assert_contains 'Treat older registered Codex copies as stale' "$ROOT_DIR/mansu-9setup/SKILL.md"
assert_contains 'installed-copy validation' "$ROOT_DIR/mansu-9setup/SKILL.md"
assert_contains 'https://github.com/bigpine86/mansu-skills.git' "$ROOT_DIR/mansu-9setup/agents/openai.yaml"
assert_contains '\$HOME/.mansu/repos/mansu-skills' "$ROOT_DIR/mansu-9setup/agents/openai.yaml"
assert_contains '\$HOME/.codex/skills/<skill-name>' "$ROOT_DIR/mansu-9setup/agents/openai.yaml"
assert_contains 'older registered Codex copies as stale' "$ROOT_DIR/mansu-9setup/agents/openai.yaml"
...
assert_contains '^## Source freshness ladder$' "$FILE"
...
assert_contains 'https://github.com/bigpine86/mansu-skills.git' "$FILE"
assert_contains '\$HOME/.mansu/repos/mansu-skills' "$FILE"
assert_contains 'If the repo is missing, clone' "$FILE"
assert_contains 'Register/sync `mansu-\*` skills' "$FILE"
assert_contains '\$HOME/.codex/skills/<skill-name>' "$FILE"
assert_contains 'Treat older registered runtime copies of Mansu-owned skills as stale' "$FILE"
assert_contains 'Existing Mansu-owned directories there' "$FILE"
assert_contains 'old installed versions are replaced by the current checkout' "$FILE"
assert_contains 're-registering all top-level `mansu-\*` directories' "$FILE"
...
assert_contains 'MANSU_COMPARE_INSTALLED=1 scripts/validate_mansu_installed_copies.sh <runtime-skill-dir>' "$FILE"
...
assert_contains 'source skill freshness for Ouroboros, gstack, Oh My / OMO / OMC, addyosmani/agent-skills style skills, and VoltAgent/awesome-design-md design references' "$FILE"
assert_contains 'VoltAgent/awesome-design-md' "$FILE"
assert_contains 'Open Design' "$FILE"
assert_contains 'Mansu runs the required CLI commands' "$FILE"
assert_contains 'Open Design / `nexu-io/open-design`' "$FILE"
assert_contains 'clone `https://github.com/nexu-io/open-design.git`' "$FILE"
assert_contains 'pnpm tools-dev' "$FILE"
assert_contains 'external callable design route' "$FILE"
assert_contains 'Keep as a design-reference family by default' "$FILE"
assert_contains 'Do not automatically install or copy VoltAgent/awesome-design-md into runtime skill directories' "$FILE"
assert_contains 'missing source tools are work to do, not just facts to report' "$FILE"
assert_contains 'Before any third-party or global install command runs, verify the install recipe' "$FILE"
assert_contains 'blocked by unverified install recipe' "$FILE"
assert_contains 'official repository documentation, official release notes, or package-manager' "$FILE"
...
assert_contains 'ouroboros setup --runtime codex --mcp-mode auto --non-interactive' "$FILE"
assert_contains 'mcp__ouroboros\.ouroboros_interview' "$FILE"
assert_contains 're-check MCP discovery with `tool_search` for `ouroboros interview mcp`' "$FILE"
assert_contains 'report that `mansu-1define` can call' "$FILE"
assert_contains 'Ouroboros Codex' "$FILE"
assert_contains 'MCP integration not exposed' "$FILE"
assert_contains 'Mansu-setup only prepares Ouroboros' "$FILE"
assert_contains 'mansu-project-start` owns Ouroboros' "$FILE"
assert_contains 'clone `https://github.com/garrytan/gstack.git`' "$FILE"
assert_contains 'LazyCodex / OMO Codex plugin' "$FILE"
assert_contains 'clone `https://github.com/code-yeongyu/lazycodex.git`' "$FILE"
assert_contains 'codex plugin add omo@sisyphuslabs' "$FILE"
assert_contains 'Disable or report telemetry hooks' "$FILE"
...
assert_contains 'Use `Check -> Decide -> Mutate` for source ecosystem work' "$FILE"
assert_contains 'If the user asks whether source tools are latest, current, stale, fresh, or' "$FILE"
assert_contains 'needs a refresh, use `source-check` first and make no changes' "$FILE"
assert_contains 'If the user explicitly says update, upgrade, sync, install missing tools, or' "$FILE"
assert_contains 'only after explicit user approval' "$FILE"
assert_contains 'reports skipped checks as yellow' "$FILE"
assert_contains 'Oh My / OMO / OMC source skill locations' "$FILE"
assert_contains 'addyosmani/agent-skills repo path' "$FILE"
assert_contains 'VoltAgent/awesome-design-md repo path' "$FILE"
assert_contains 'Open Design repo path' "$FILE"
...
assert_contains '| Codex | `codex` | `$HOME/.codex/skills` | LazyCodex / OMO Codex plugin (`omo@sisyphuslabs`) for Codex-side Oh My routes; `omx` as legacy compatibility tooling when present |' "$FILE"
assert_contains '| Claude Code | `claude-code` | `$HOME/.claude/skills` when present | `omc` optional compatibility tooling |' "$FILE"
assert_contains 'LazyCodex / OMO Codex plugin (`omo@sisyphuslabs`) plus `omx` / Oh My Codex compatibility check when present' "$FILE"
assert_contains 'check or update LazyCodex / OMO Codex plugin and report legacy `omx` compatibility separately' "$FILE"
...
assert_contains 'whether Codex can discover' "$FILE"
...
assert_contains 'MANSU SETUP 보고서' "$FILE"
...
assert_contains '설치본 검증:' "$FILE"
assert_contains '건너뛴 확인:' "$FILE"
assert_contains "End with this report in the user's language or the project's default language" "$FILE"
assert_contains 'translate labels when' "$FILE"
...
assert_contains '$HOME/.config/opencode/skills/mansu-setup' "$README_FILE"
assert_contains '.opencode/skills/mansu-setup' "$README_FILE"
...
assert_contains 'display_name: "Mansu Setup"' "$OPENAI_YAML"
assert_contains 'short_description: "Runtime-aware Mansu bootstrap and updater"' "$OPENAI_YAML"
assert_contains 'default_prompt: "Use \$mansu-setup' "$OPENAI_YAML"
assert_contains 'detect the runtime target, agent host, and OS' "$OPENAI_YAML"
assert_contains 'clone https://github.com/bigpine86/mansu-skills.git into the canonical local Mansu repo path' "$OPENAI_YAML"
assert_contains 'sync and register Mansu skills' "$OPENAI_YAML"
assert_contains 'replace stale Mansu-owned copies in the selected Codex skill directory' "$OPENAI_YAML"
...
assert_contains 'install or update Ouroboros, gstack, and the matching Oh My adapter when explicitly needed' "$OPENAI_YAML"
assert_contains 'mcp__ouroboros\.ouroboros_interview is discoverable' "$OPENAI_YAML"
assert_contains 'verify with tool_search that mcp__ouroboros\.ouroboros_interview is discoverable' "$OPENAI_YAML"
assert_contains 'then re-check tool_search before syncing' "$OPENAI_YAML"
assert_contains 'ouroboros setup --runtime codex --mcp-mode auto --non-interactive' "$OPENAI_YAML"
assert_contains 'prepare Open Design only when UI/design artifact work needs it or the user approves it' "$OPENAI_YAML"
assert_contains 'run Open Design CLI/lifecycle commands directly when approved' "$OPENAI_YAML"
assert_contains 'hand off source-reference drift to \$mansu-source-curator' "$OPENAI_YAML"
assert_contains 'verify Ouroboros, gstack, Oh My / OMO / OMC, addyosmani/agent-skills, VoltAgent/awesome-design-md, and Open Design source freshness' "$OPENAI_YAML"
