...
assert_contains '| Codex | `codex` | `$HOME/.codex/skills` | LazyCodex / OMO Codex plugin (`omo@sisyphuslabs`) preferred inside the Oh My family; `omx` fallback compatibility tooling |' "$FILE"
assert_contains '| Claude Code | `claude-code` | `$HOME/.claude/skills` when present | `omc` optional compatibility tooling |' "$FILE"
assert_contains 'LazyCodex / OMO Codex plugin (`omo@sisyphuslabs`) first, then `omx` / Oh My Codex' "$FILE"
assert_contains 'check or update LazyCodex / OMO Codex plugin first and treat `omx` as fallback' "$FILE"
