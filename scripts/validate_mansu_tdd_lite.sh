...
grep -q 'carry the exact gstack/current-runtime gate' "$FILE"
grep -q 'gstack-review' "$FILE"
grep -q 'gstack-qa-only' "$FILE"
grep -q 'gstack-context-save' "$FILE"
grep -q 'A builder saying' "$FILE"
...
grep -q 'review, QA, and checkpoint are complete with separately invoked gate evidence' "$FILE"
