...
grep -q 'Ouroboros `interview` vs ULW deep interview vs gstack interview-like skills' "$FILE"
grep -q 'Use Ouroboros as the definition source' "$FILE"
grep -q 'Use ULW deep interview only in public `mansu-2plan`' "$FILE"
grep -q 'Use gstack interview-like skills' "$FILE"
grep -q 'Do not let ULW deep interview or gstack replace the Ouroboros definition gate' "$FILE"
...
grep -q 'Dense plan after definition: public `mansu-1define` with Ouroboros evidence' "$FILE"
grep -q 'explicit OMO planning artifact or ULW deep interview only when the user invoked it' "$FILE"
...
grep -q 'ULW deep interview / `ulw-plan`: plan-density support after definition' "$CATALOG"
grep -q 'Ouroboros interview is the first definition interview' "$CATALOG"
grep -q 'ULW deep interview belongs in the Plan layer' "$CATALOG"
grep -q 'Do not substitute ULW deep interview or gstack interview-like consultation' "$CATALOG"
