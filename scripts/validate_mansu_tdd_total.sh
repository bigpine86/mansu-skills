...
grep -q '^## Accepted plan requirements$' "$FILE"
...
grep -q 'Synthesizer: merge critique into one accepted plan' "$FILE"
grep -q 'record the mapping in `PLAN.md`' "$FILE"
grep -q 'Do not proceed to implementation until the accepted plan and slice table are visible' "$FILE"
