...
3. Codex에서 LazyCodex / OMO가 설치되어 있으면 Oh My 계열 실행을 우선 맡기고, 초기 목표는 Ouroboros, 검증은 gstack, 디자인 산출물/참조는 Open Design과 design-md, 라우팅/증거/최종 판단은 Mansu가 맡는다.
4. skill target confidence가 낮으면 쓰지 말고 `check` 모드로 보고한다.
5. `review` / `qa` / `checkpoint` 같은 gate family는 특정 역사적 tool name보다 우선한다.
6. validator는 wording뿐 아니라 stale runtime assumptions도 잡아야 한다.
