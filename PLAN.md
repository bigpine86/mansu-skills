...
사용자가 기억하고 호출해야 하는 lifecycle skill은 이 흐름에 맞춰 6개로 줄인다.
다만 실제 public surface는 entry/setup/debug를 포함한다:
`mansu-help`, `mansu-setup`, `mansu-define`, `mansu-plan`, `mansu-build`,
`mansu-verify`, `mansu-review`, `mansu-debug`, `mansu-ship`.
...
- `mansu-define`: 제품 목표, 요구사항, 초기 범위, 성공 기준을 잡는다.
- `mansu-plan`: 스펙, 아키텍처, 디자인 방향, 구현 순서, phase roadmap을 만든다.
  디자인이 필요한 제품/앱/웹 작업에서는 design source skill route를 반드시 포함한다.
- `mansu-build`: 구현을 진행한다. TDD slice, source skill 실행, checkpoint를 담당한다.
- `mansu-verify`: 테스트, 브라우저 검증, QA 증거, 회귀 확인을 담당한다.
- `mansu-review`: 코드 리뷰, 설계 리뷰, 문제 원인 분석, 수정/개선 loop를 담당한다.
- `mansu-ship`: 릴리스 gate, 문서, 공유, 배포/보류 판단을 담당한다.
...
| --- | --- |
| `mansu-project-start` | `mansu-define`의 호환 alias. 실제 시작은 Ouroboros interview/Seed/Ledger를 먼저 사용한다. |
| `mansu-operating-model` | `mansu-plan`의 내부 reference/runtime model. 직접 호출보다 plan phase가 사용한다. |
| `mansu-tdd-total` | `mansu-build`의 기본 implementation route. 필요하면 lite/strict mode를 내부 선택한다. |
| `mansu-tdd-lite` | `mansu-build` 내부 fast mode. |
| `mansu-tdd-strict` | `mansu-build` 내부 strict mode. |
| `mansu-web-verify` | `mansu-verify`의 브라우저/프론트 검증 route. |
| `mansu-debug-rootcause` | `mansu-debug`의 호환 alias. Review 중 발견된 결함도 원인 불명일 때는 이 route로 점프한다. |
| `mansu-ship-release` | `mansu-ship`의 호환 alias 또는 내부 release route. |
| `mansu-setting` | `mansu-setup` 호환 alias. 유지하되 새 문서에서는 숨긴다. |

## Plan Phase Internal Order

`mansu-plan`은 `Design`을 별도 최상위 phase로 분리하지 않는다.
대신 Plan phase 안의 필수 gate로 둔다.

`Requirements/Spec -> Architecture -> Design Direction -> Implementation Roadmap`

- Requirements/Spec: 무엇을 만들지, 요구사항과 성공 기준을 정리한다.
- Architecture: 시스템 구조, 데이터 흐름, 주요 기술 선택을 정리한다.
- Design Direction: 사용자-facing 제품에서는 reference discovery, 디자인 방향 후보, 디자인 리뷰를 source skill로 수행한다.
- Implementation Roadmap: 위 결정을 바탕으로 phase별 작업 단위와 검증 기준을 만든다.
...
아니라 Codex runtime transport readiness로만 다룬다.

| Phase | Internal order | Primary source skills |
| --- | --- | --- |
| `mansu-define` | Idea intake -> User/problem interview -> Requirements -> Acceptance criteria -> Source-of-truth seed | Ouroboros `interview`, `pm`, `seed`, `brownfield`; addyosmani `interview-me`, `idea-refine`, `spec-driven-development`; gstack `office-hours`, `plan-ceo-review` |
| `mansu-plan` | Requirements/Spec -> Architecture -> Design Direction -> Implementation Roadmap | addyosmani `spec-driven-development`, `planning-and-task-breakdown`, `api-and-interface-design`, `source-driven-development`; OMO `ulw-plan`; gstack `autoplan`, `plan-ceo-review`, `plan-eng-review`, `plan-devex-review`; design route below |
| `mansu-plan` design gate | Reference discovery -> Design system/artifact -> Variant exploration -> Plan-stage design review -> Implementation handoff | OMO research/deepsearch when available; Open Design for callable design artifacts; VoltAgent/awesome-design-md for `DESIGN.md` grammar; gstack `design-consultation`, `design-shotgun`, `plan-design-review`, `design-html`; addyosmani `frontend-ui-engineering` |
| `mansu-build` | Context pack -> Contract/source verification -> Slice plan -> Implement -> Local validation -> Checkpoint | `mansu-tdd-total` as the internal build engine under `mansu-build`; addyosmani `context-engineering`, `source-driven-development`, `incremental-implementation`, `test-driven-development`, `frontend-ui-engineering`; gstack `guard`, `careful`, `checkpoint` |
| `mansu-verify` | Test evidence -> Runtime/browser evidence -> QA report -> Security/perf checks when relevant -> Verification closeout | addyosmani `test-driven-development`, `browser-testing-with-devtools`, `security-and-hardening`, `performance-optimization`; gstack `qa-only`, `qa`, `browse`, `health`, `cso`, `benchmark`; Ouroboros `qa`, `evaluate`; OMO `review-work` QA lane |
...
- LazyCodex is explicitly excluded from source-skill composition and belongs only
  to runtime transport readiness under `mansu-setup`.
...
| --- | --- | --- |
| addyosmani `interview-me` vs Ouroboros `interview` vs gstack `office-hours` | `interview-me` extracts intent; Ouroboros creates durable Seed/Ledger/project definition; gstack tests product demand and wedge | If the user's intent is unclear, start with `interview-me`. If the project needs long-lived definition, use Ouroboros. If product value is questionable, add gstack `office-hours`. |
| addyosmani `spec-driven-development` vs OMO `ulw-plan` vs gstack `autoplan` | addyosmani writes the spec; OMO makes a decision-complete execution plan; gstack critiques the plan from specialist roles | Spec first, execution plan second, independent review third. Do not let a plan review replace the spec. |
| Open Design vs VoltAgent/awesome-design-md vs gstack design skills vs OMO `frontend-ui-ux` | Open Design creates visible artifacts; design-md gives `DESIGN.md` grammar; gstack explores/critiques design; OMO implements refined UI | For UI, choose design route inside `mansu-plan` before build. Use Open Design when artifact/prototype matters, design-md for durable design docs, gstack for taste/review, OMO for implementation. |
| Runtime transport vs source skills | LazyCodex/OMO is the Codex runtime transport layer; addyosmani, Ouroboros, gstack, Open Design, and design-md are the source skill families Mansu composes | Do not present LazyCodex as a Mansu source skill family. Keep it in `mansu-setup` runtime readiness and let phase skills focus on source-skill orchestration. |
| Verify vs Review source skills | Verify proves behavior; Review judges quality, safety, maintainability, and decision risk | Keep them as separate evidence gates, but execute them as a continuous `mansu-verify -> mansu-review` sequence by default when verification passes. Do not count self-review as QA. |
...
- Complex implementation: addyosmani `planning-and-task-breakdown` -> gstack `autoplan` -> `mansu-build`/`mansu-tdd-total`.
- User-facing UI: research/deepsearch when needed -> Open Design or gstack `design-shotgun`/`design-consultation` -> design-md-shaped `DESIGN.md` -> gstack `plan-design-review`.
- Unknown bug: OMO `debugging` -> gstack `investigate` -> regression proof -> minimal fix -> `mansu-verify`.
- Ship candidate: `mansu-verify` evidence -> `mansu-review` evidence -> gstack `ship`/`document-release` -> `mansu-ship` verdict.

## Source Skill Responsibilities

- Define: Ouroboros PM/interview/Seed/Ledger가 초기 목표와 문제 정의를 이끈다.
- Plan: addyosmani define/plan/build phase skill을 큰 지도 삼고, Mansu가 스펙, 아키텍처, 디자인 방향, phase roadmap으로 변환한다.
- Design planning: `mansu-plan` 안의 필수 route다. Oh My/OMO research/deepsearch로 reference를 찾고, gstack design consultation/shotgun으로 여러 방향을 만든 뒤, gstack plan-design-review/design-review로 계획 단계에서 비평한다.
- Build: `mansu-build`는 기존 `mansu-tdd-total`을 계승하고, 구현 discipline은 addyosmani/gstack source skill 조합으로 잡는다. LazyCodex는 source skill family가 아니라 `mansu-setup`의 Codex runtime transport readiness로만 다룬다.
...
- `mansu-build` should preserve the existing `mansu-tdd-total` behavior rather
  than rewrite it.
...
| `mansu-define` | `SOURCE_SKILL_COMPOSITION.md`, Ouroboros skills, addyosmani interview/spec skills, gstack office-hours | interview and define purpose, user, problem, success criteria, constraints, source of truth | `AGENTS.md`, `CODING_RULES.md`, project brief/spec, Ouroboros Seed/Ledger reference when available |
| `mansu-plan` | define outputs, `DOCUMENT_CREATION_ORDER.md`, `CODE_CONSTRUCTION_ORDER.md`, design route docs | produce full project phase map and current phase implementation plan | requirements/spec, architecture notes, design direction, implementation roadmap, phase-level `PLAN.md` |
| `mansu-build` | current phase `PLAN.md`, `mansu-tdd-total`, source composition rules | execute the current phase through TDD slices | updated slice table, validation evidence, checkpoint/worklog/commit status |
| `mansu-verify` | changed surface, tests, runtime/browser paths, QA source skills | prove behavior with tests, browser/runtime evidence, QA/security/perf checks when relevant; when verification passes, continue directly into `mansu-review` by default | verification report, pass/fail evidence, untested risk, review handoff result |
| `mansu-review` | diff, plan, verification report, review source skills | judge quality, maintainability, security, design, and decision risk | review findings, required fixes, reverify route |
...
| `mansu-ship` | verify/review evidence, release docs, git/deploy state | decide ship/hold and prepare release/deploy/learning handoff | ship/hold verdict, release notes, PR/deploy/canary/learning evidence |
...
- `mansu-project-start` routes to `mansu-define`.
- `mansu-tdd-total` remains the implementation engine under `mansu-build`.
- `mansu-web-verify` routes to `mansu-verify`.
- `mansu-debug-rootcause` routes to `mansu-debug`.
- `mansu-ship-release` routes to `mansu-ship`.
...
- Validator: add a source-composition validator that checks the reference exists,
  mentions LazyCodex only as runtime transport, and preserves the key comparison
  pairs.
...
- Add `mansu-define`, `mansu-plan`, `mansu-build`, `mansu-verify`,
  `mansu-review`, `mansu-debug`, and `mansu-ship` as user-facing entry skills.
...
- Add BDD checks for:
  ...
  `mansu-define` interview/source-of-truth outputs,
  `mansu-plan` phase roadmap and design gate,
  `mansu-build` delegating to `mansu-tdd-total`,
...
- The manual must reflect the final implemented surface, not the intended
  ...
  surfaces, alias policy, source family map, and LazyCodex as runtime transport
  readiness only.
...
- `mansu-setup` treats LazyCodex as runtime transport readiness, not source craft.
- `mansu-build` preserves `mansu-tdd-total` as the default build route.
...
- `mansu-manual` is updated last and matches the final implemented skill surface,
  source composition map, alias policy, and runtime transport distinction.
...
Scenario: New fuzzy product
  ...
  Expected: recommends mansu-define; mentions interview/source-of-truth before planning.
...
Scenario: Existing phase plan ready to build
  ...
  Expected: recommends mansu-build and delegates to mansu-tdd-total.
  Evidence: validator output under scripts/validate_mansu_phase_routes.sh.
...
Scenario: Source refresh
  ...
  Expected: source state is current; LazyCodex appears only as runtime transport readiness.
  Evidence: ./scripts/validate_mansu_source_lock.sh and source-composition validator output.
```

## Migration Steps

1. Add the six phase skills: `mansu-define`, `mansu-plan`, `mansu-build`, `mansu-verify`, `mansu-review`, `mansu-ship`.
...
- Given a new product idea, when the user says `mansu define` or `mansu project start`, then Mansu uses Ouroboros interview/Seed/Ledger before writing Mansu planning docs.
- Given an existing idea with unclear design direction, when the user asks to plan, then `mansu-plan` routes design discovery through Oh My/OMO research and gstack design skills instead of asking ad hoc design questions itself.
- Given a user-facing app or website, when `mansu-plan` writes the implementation roadmap, then the plan includes design reference discovery, design direction candidates, and plan-stage design review evidence.
- Given an approved plan, when the user asks to build, then Mansu uses `mansu-build`, delegates to `mansu-tdd-total`, and records the source skills and validation gates used for the current slice.
- Given completed implementation, when the user asks to verify, then Mansu runs tests/browser/QA evidence through `mansu-verify`.
...
- Given verified work, when the user asks to ship, then Mansu uses a release gate and clearly returns ship/hold evidence.
...
- Future follow-up: exercise `mansu-define` on a real greenfield project and
  `mansu-ship` on a real release to tune report shape.
...
- Updated `mansu-setup` to prepare Ouroboros, gstack, and the matching Oh My adapter, while leaving actual Ouroboros project-definition work to `mansu-project-start`.
