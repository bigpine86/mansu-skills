...
| Open Design vs VoltAgent/awesome-design-md vs gstack design skills vs OMO `frontend-ui-ux` | Open Design creates visible artifacts; design-md gives `DESIGN.md` grammar; gstack explores/critiques design; OMO implements refined UI | For UI, choose design route inside public `mansu-2plan` before build. Use Open Design when artifact/prototype matters, design-md for durable design docs, gstack for taste/review, OMO for implementation. |
...
- Fuzzy new product: addyosmani `interview-me` -> gstack `office-hours` -> Ouroboros `interview`/`seed` -> `mansu-2plan`.
...
| `mansu-1define` | `SOURCE_SKILL_COMPOSITION.md`, Ouroboros skills, addyosmani interview/spec skills, gstack office-hours; backing implementation `mansu-define` | interview and define purpose, user, problem, success criteria, constraints, source of truth | `AGENTS.md`, `CODING_RULES.md`, project brief/spec, Ouroboros Seed/Ledger reference when available |
| `mansu-2plan` | define outputs, `DOCUMENT_CREATION_ORDER.md`, `CODE_CONSTRUCTION_ORDER.md`, design route docs; backing implementation `mansu-plan` | produce full project phase map and current phase implementation plan | requirements/spec, architecture notes, design direction, implementation roadmap, phase-level `PLAN.md` |
